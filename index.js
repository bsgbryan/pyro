...
(function() {
  var Firebase, add_leaf, add_value, async, biggest, count, counts, execute_query, exist, exists, fetch, find, firebase, get, increment_count, monitor, observe, priority, push, q, remove, sanitize, set, smallest, to, touch, unmonitor, unwatch, watch,
    __slice = [].slice;
  ...
  fetch = function(path, end) {
    var callback, deferred, fetched, items, limit;
    deferred = q.defer();
    fetched = 0;
    limit = 0;
    items = [];
    callback = function(snapshot) {
      ...
      if (++fetched === limit) {
        ...
        return firebase.child(sanitize(path)).off('child_added', callback);
      }
    };
    count(path, end).then(function(cnt) {
      limit = cnt;
      return firebase.child(sanitize(path)).on('child_added', callback);
    });
    return deferred.promise;
  };
  ...
  to = function(path, end, limit) {
    var callback, deferred, invokes, out;
    deferred = q.defer();
    invokes = 0;
    out = [];
    callback = function(snapshot) {
  ...
  counts = function() {
    var deferred, out, paths;
    paths = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    deferred = q.defer();
    out = {};
    async.eachSeries(paths, function(path, next) {
      return firebase.child(sanitize(path)).once('value', function(snapshot) {
  ...
  observe = function(path, event) {
    var deferred;
    deferred = q.defer();
    firebase.child(sanitize(path)).on("child_" + event + "ed", function(snapshot) {
      return deferred.notify({
        name: snapshot.key(),
        value: snapshot.val(),
        priority: snapshot.getPriority()
